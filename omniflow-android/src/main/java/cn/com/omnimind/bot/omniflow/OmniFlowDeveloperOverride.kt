...
internal class OmniFlowDeveloperOverrideStore(
    ...
    fun status(runtimeVersion: String): OmniFlowDeveloperOverrideStatus {
        ...
        return OmniFlowDeveloperOverrideStatus(
            enabled = pythonRoot.isDirectory && modifiedPaths.isNotEmpty(),
    ...
    fun apply(
        ...
        require(target.canonicalPath.startsWith(pythonRoot.canonicalPath + File.separator)) {
    ...
    private fun ensureBase(basePythonRoot: File, runtimeVersion: String) {
        require(basePythonRoot.isDirectory) { "omniflow_override_base_missing" }
        if (pythonRoot.isDirectory && versionFile.takeIf(File::isFile)?.readText()?.trim() == runtimeVersion) return
        ...
        require(basePythonRoot.copyRecursively(File(temporary, "python"), overwrite = true)) {
...
internal fun normalizedPythonPath(value: String): String {
    ...
    require(path.matches(Regex("[A-Za-z0-9_./-]+\\.py"))) {
        "omniflow_override_path_invalid"
    }
    return path
}
