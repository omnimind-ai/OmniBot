package cn.com.omnimind.bot.storage
...
/** Shared storage analysis and cleanup owner for Compose and the legacy Flutter channel. */
class StorageUsageRepository private constructor(context: Context) {
    private val appContext = context.applicationContext
    private val mutex = Mutex()

    suspend fun summary(): Map<String, Any?> = withContext(Dispatchers.IO) {
        mutex.withLock { analyzeStorageUsage(appContext, persistSnapshot = true).summary }
    }

    suspend fun clear(categoryId: String, olderThanDays: Int?): Map<String, Any?> = withContext(Dispatchers.IO) {
        require(categoryId.isNotBlank()) { "categoryId is required" }
        mutex.withLock { clearCategory(appContext, categoryId.trim(), olderThanDays?.takeIf { it > 0 }) }
    }

    suspend fun applyStrategy(strategyId: String, olderThanDays: Int?, targetReleaseBytes: Long): Map<String, Any?> =
        withContext(Dispatchers.IO) {
            require(strategyId.isNotBlank()) { "strategyId is required" }
            mutex.withLock { applyCleanupStrategy(appContext, strategyId.trim(),
                olderThanDays?.takeIf { it > 0 }, targetReleaseBytes) }
        }

    companion object {
        private const val TAG = "StorageUsageRepository"
        ...
        @Volatile private var instance: StorageUsageRepository? = null
        fun get(context: Context): StorageUsageRepository = instance ?: synchronized(this) {
            instance ?: StorageUsageRepository(context).also { instance = it }
        }
    }
