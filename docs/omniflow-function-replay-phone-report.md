...
为避免把 Function Replay 的结论外推到所有 App，又在同一台手机上执行了两个只读在线任务。它们不是 Transfer 内核基准，而是用来观察页面前置条件、云端 VLM 与真实 App 状态对 wall time 的影响。

| 场景 | 结果 | 可确认数据 | 失败/成功边界 |
| --- | --- | --- | --- |
| 百度地图搜索上海附近咖啡店 | 成功 | UI 计时约 `72 s`；1 次模型调用；进入 `com.baidu.BaiduMap` 搜索结果页 | 未导航、未拨号；该轮是在线 VLM，不包含 Function Transfer |
| 美团搜索上海餐厅 | 未完成 | UI 计时约 `109 s`；先遇到验证/中间 Activity，随后尝试返回主界面并点击搜索栏 | 未下单、未支付；任务在搜索完成前终止，不能计入成功率 |

这两个结果补充说明了一个重要边界：在线路径的主要瓶颈可以是云端响应、验证页和 App 页面状态，而不一定是 Transfer。美团这次没有把验证页“绕过去”，也没有加入 renew/reopen 或隐式重试；它按真实页面状态结束为未完成，保留为失败样本。

### 4.9 复杂组合任务的当前结果

又执行了一次“三次连续只读搜索”的复杂小红书在线任务（AI agents、machine learning、smartphone）。模型接受了任务，但第一步工具执行在约 `105 s` 后仍停留在 `in_progress`，在额外观察窗口后由官方“停止工具”取消；没有形成三次搜索的完成证据，也没有点赞、关注、评论、分享或购买。该样本说明复杂组合的主要风险目前是在线 Agent 工具编排和页面状态推进，而不是单次 Transfer 矩阵规模；在没有官方 Function RunLog 的情况下，不能把它伪装成 Replay 结果。
...
| 在线百度地图搜索 | 1 | 1 | 未按 Function 计步 | 1 | 0 | 约 72 s |
| 在线美团搜索 | 1 | 0 | 未完成 | 未从 UI 可靠提取 | 未知 | 约 109 s |
| 在线小红书三次连续搜索 | 1 | 0 | 未完成，官方取消 | 未可靠提取 | 未知 | 约 130 s 后取消 |
